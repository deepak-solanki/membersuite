...
class Concierge {
    ...
    protected function createobject($xmlresponse, $method) {//echo $xmlresponse;exit;
            ...
            if ($key == 'ClassType') {
                ...
                if (key($value) === 'RecordType') {
                    ...
                    foreach ($value as $k => $v) {
                        if ($k !== 'RecordType') {
                            $arrData .= '<mem:' . $k;
                            if (!is_array($v) && strlen($v) > 0) $arrData .= '>' . $v . '</mem:' . $k . '>';
                            else $arrData .= ' i:nil="true" />';
                        }
                    }
    ...
    protected function object_to_array($data) {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_array($value)) $result[$key] = $this->object_to_array($value);
    ...
    protected function build_msobject($data) {
        if (property_exists($data, 'bClassType') && property_exists($data, 'bFields') && property_exists($data->bFields, 'bKeyValueOfstringanyType')) {
            $obj = new stdClass();
